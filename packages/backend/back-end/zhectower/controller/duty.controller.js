...
function GetWhoIsOnDuty(req, res) {
    ...
    // if (dutyType) {
    //     // Split the 'position' input string by ';'
    //     const dutyTypeArray = dutyType.split(",").map((p) => p.trim()); // Trim to avoid leading/trailing spaces
    //     console.log(dutyTypeArray);

    //     for (let index = 0; index < dutyTypeArray.length; index++) {
    //         const d = dutyTypeArray[index];
    //         if (index === 0 && p !== "") {
    //             query += ` AND (dutyType LIKE ?`;
    //             params.push(`%${d}%`);
    //         } else if (index > 0 && p !== "") {
    //             query += ` OR dutyType LIKE ?`;
    //             params.push(`%${d}%`);
    //         } else {
    //         }
    ...
    if (outTime || (startDate && startTime)) {
        const _outTime =
...
module.exports = { GetWhoIsOnDuty, PostToTakeOverDuty, NoPrepareTableCheckPostToTakeOverDuty, PutToOutDuty };
