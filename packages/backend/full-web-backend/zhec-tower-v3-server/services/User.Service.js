...
class UserService extends BaseService {
    ...
    //     let users = await this.db.findAll(orderBy);
    //     let result = [];
    //     for (let user of users) {
    //         let duty = await DutyDb.findById(user.dutyId);
    //         result.push({
    //             ...user,
    //             duty: duty
    //         });
    ...
    getAll({ fields, groupBy }) {
        ...
        return new Promise((resolve, reject) => {
            const safeFields = fields[0] === "*" ? ALLOWED_COLUMNS : fields.filter((f) => ALLOWED_COLUMNS.includes(f));

            const query = `SELECT ${safeFields.join(", ")} FROM user ORDER BY "order"`;
