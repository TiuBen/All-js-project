    ...
    <Card className="border-amber-200 bg-amber-50/40">
      <CardContent className="p-4">
        <div className="mb-3 flex items-center gap-2">
          ...
          <span className="text-[11px] text-slate-400">点击继续编辑，最多保留 5 份</span>
        </div>
          ...
          {drafts.map((d) => (
            ...
            >
              ...
              <div className="mt-2 flex items-center gap-1 text-[11px] text-slate-400">
                <span className="rounded bg-slate-100 px-1.5 py-0.5">{d.templateId === 'passenger-checklist' ? '客运' : '货运'}</span>
                <span className="truncate">{d.templateId === 'passenger-checklist' ? '客运检查单' : '货运检查单'}</span>
              </div>
              <div className="mt-3">
