...
/**
 * ============================================================
 * CargoInitFlight —— 货运始发航班 检查单面板组
 * ------------------------------------------------------------
 * 「穷举法」：一个检查单类型 = 一个组件，由 Template/index.js 的
 * `PANEL_BY_CATEGORY` 按 category 查表选中，编译期确定。
 *
 * ★ 本文件唯一职责：**声明这一类型用哪份数据**，再把它交给三个面板
 *   <MainMonitoringPanel source={cargoInitFlight} />  → 主监控节点取 source.schema
 *   <AuxiliaryPanel     source={cargoInitFlight} />  → 辅助项取选中节点的 auxiliaries
 *   <VideoPanel         source={CARGO_VIDEO_FOCUS} /> → 货运两类共用同一份视频监管
 *
 *   三个面板其余状态全部自己从 store 订阅，不在这里透传。
 * ============================================================
 */
export default function CargoInitFlight() {
    ...
    return (
        <ResizableColumns
            ...
            columns={[
                ...
                panels.includes("main") && { key: "main", content: <MainMonitoringPanel source={cargoInitFlight} /> },
                panels.includes("aux") && { key: "aux", content: <AuxiliaryPanel source={cargoInitFlight} /> },
