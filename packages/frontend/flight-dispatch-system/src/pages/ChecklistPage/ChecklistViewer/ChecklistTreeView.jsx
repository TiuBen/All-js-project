...
export default function ChecklistTreeView({ template, record }) {
    ...
    // 记录不再存 flight_type：检查单类型由 header.template.checklistName 反查（始发/过站/顺航/常规）
    const flightType = record?.header?.template?.checklistName || template?.checklistName || "常规";
    ...
    const colCls =
        cols === 1 ? "columns-1" : cols === 2 ? "columns-1 md:columns-2" : "columns-1 md:columns-2 xl:columns-3";
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        {n.responsible && (
                                            <span className="shrink-0 rounded bg-violet-50 px-1 py-0.5 text-[9px] text-violet-600">
                                                {n.responsible}
                                            </span>
                                        )}
                                        {mainItem.time && (
                                    ...
                                    {mainItem.note && (
                                        <div className="whitespace-normal break-words px-11 pb-1 text-[11px] text-slate-500">
                                            {mainItem.note}
                                        </div>
                                    )}
