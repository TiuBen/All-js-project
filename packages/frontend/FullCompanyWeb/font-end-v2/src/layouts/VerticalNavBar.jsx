...
function VerticalNavBar() {
    return (
        <nav className="flex flex-col flex-shrink-0  justify-between w-34 bg-zinc-100   divide-y-2 divide-slate-700 overflow-auto">
            <section className="flex-1 align-self-start">
                <ul className="flex flex-col ">
                    {allNavItem.map((item, innerIndex) => {
                        return (
                            <VerticalNavItem
                                key={innerIndex}
                                icon={item.icon}
            ...
            <section className="align-self-end p-0 text-blue-900 font-yahei whitespace-nowrap">
                <ul className="flex flex-col flex-1 items-stretch justify-around ">
                    {[
                        { title: "鼎道", baseUrl: "dd" },
                        { title: "韩晶威", baseUrl: "hjw" },
                        { title: "安文", baseUrl: "aw" },
                        { title: "海迪龙", baseUrl: "hdl" },
                    ].map((company, index) => {
                        return (
                            <li key={"section2" + index} className="px-2 flex flex-row items-center text-lg">
                                <input type="checkbox" id={company.title + index} value={company.title} />
                                <label
                                    name={company.title + index}
                                    htmlFor={company.title + index}
                                    className="pl-1 w-full text-[16px] font-semibold  "
                                >
                                    {company.title}
                                </label>
                                <Link
                                    className="m-0 p-0 flex flex-row items-center hover:underline"
                                    to={`/company?name=${company.title}`}
                                >
